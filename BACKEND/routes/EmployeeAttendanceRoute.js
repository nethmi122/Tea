  ...
  try {
    if (
      ...
      return response.status(400).send({
        message: 'Send all required fields: Emp_ID, Name, date, InTime, OutTime, OThours',
      });
    }
    ...
    const employeeattendance = await EmployeeAttendence.create(newEmployeeAttendence);

    return response.status(201).send(employeeattendance);
  } catch (error) {
...
router.get('/', async (request, response) => {
  try {
    const employeeattendance = await EmployeeAttendence.find({});
...
router.get('/:id', async (request, response) => {
  try {
    ...
    const employeeattendance = await EmployeeAttendence.findById(id);

    return response.status(200).json(employeeattendance);
  } catch (error) {
    console.log(error.message);
    response.status(500).send({ message: error.message });
  }
...
//       const employeeAttendances = await EmployeeAttendence.find({ Emp_ID: id });
...
router.put('/:id', async (request, response) => {
  try {
    ...
    if (request.body.Emp_ID) {
      existingAttendance.Emp_ID = request.body.Emp_ID;
    }
    if (request.body.Name) {
      existingAttendance.Name = request.body.Name;
    }
...
// Route for Delete an employeeattendance
router.delete('/:id', async (request, response) => {
  try {
    ...
    if (!result) {
      return response.status(404).json({ message: 'Employee Attendance not found' });
    }

    return response.status(200).json({ message: 'Employee Attendance deleted successfully' });
  } catch (error) {
    console.error(error.message);
...
router.get("/searchEmployeeAttendence", async (req, res) => {
  try {
    ...
    const { page = 1, limit = 7, search = "", sort = "Emp_ID" } = req.query;
